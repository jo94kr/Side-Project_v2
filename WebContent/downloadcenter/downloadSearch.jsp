...
<body>
	<div id="wrap">
		...
		<%
			...
			int count = ddao.getFileCount(search);
			// 한 화면에 보여줄 글개수 설정
			int pageSize = 15;
			// 현페이지 가져오기
			...
			List downloadList = ddao.getFileList(startRow, pageSize, search);
			// 날짜형태 변경 => 문자열 변경
		...
		<article>
			...
			<table id="notice">
				...
				%>
				<tr onclick="location.href='downloadContent.jsp?num=<%=db.getNum()%>&pageNum=<%=pageNum%>'">
					<td><%=num--%></td>
			...
			%>
			<div id="table_search">
				<input type="button" value="글쓰기" class="btn" onclick="location.href='downloadUpload.jsp'">
			</div>
			<%
				}
			%>
			...
			<div id="page_control">
				...
				%><a href="downloadBoard.jsp?pageNum=<%=startPage - pageBlock%>&search=<%=search%>">Prev</a>
				<%
					}
					//  1 2 3 .... 10      11 12 13 .... 20
					for (int i = startPage; i <= endPage; i++) {
				%><a href="downloadBoard.jsp?pageNum=<%=i%>&search=<%=search%>"><%=i%></a>
				<%
					}
					// [다음]
					if (endPage < pageCount) {
				%><a href="downloadBoard.jsp?pageNum=<%=startPage + pageBlock%>&search=<%=search%>">Next</a>
				<%
					}
				%>
			</div>
