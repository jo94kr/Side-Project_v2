...
<body>
	<div id="wrap">
		...
		<%
			...
			// select * from gallery order by num desc  첫행 ,몇개
			// select * from gallery order by num desc  limit 첫행-1 ,몇개
			// List boardList=bdao.getBoardList();
			// List boardList=bdao.getBoardList(첫행,몇개);
			List downloadList = ddao.getFileList(startRow, pageSize);
			// 날짜형태 변경 => 문자열 변경
		...
		<article>
			...
			%>
			<div id="table_search">
				<input type="button" value="글쓰기" class="btn" onclick="location.href='downloadWrite.jsp'">
			</div>
			<%
				}
			%>
			...
			<div id="page_control">
				...
				%><a href="downloadBoard.jsp?pageNum=<%=startPage - pageBlock%>">Prev</a>
				<%
					}
					//  1 2 3 .... 10      11 12 13 .... 20
					for (int i = startPage; i <= endPage; i++) {
				%><a href="downloadBoard.jsp?pageNum=<%=i%>"><%=i%></a>
				<%
					}
					// [다음]
					if (endPage < pageCount) {
				%><a href="downloadBoard.jsp?pageNum=<%=startPage + pageBlock%>">Next</a>
				<%
					}
				%>
			</div>
