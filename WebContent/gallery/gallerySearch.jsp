...
<body>
	<div id="wrap">
		...
		<%
			...
			// 검색어 포함 게시판 전체 글개수  가져오기  select count(*)
			...
			List galleryList = gdao.getGalleryList(startRow, pageSize, search);
			// 날짜형태 변경 => 문자열 변경
		...
		<article>
			...
			<table id="notice">
				<%
					...
					int col = 5; // 게시판의 열
					if (count != 0) {
						for (int i = 0; i < galleryList.size(); i++, row++) {
							if (row % col == 1) {
				%>
				<tr>
					<%
					...
					%>
					<td><img src="../upload/<%=gb.getImage()%>" width="100" height="100" onclick="location.href='galleryContent.jsp?num=<%=gb.getNum()%>&pageNum=<%=pageNum%>'"></td>
					<%
						if (row % col == 0) {
					%>
				</tr>
			...
			%>
			<div id="table_search">
				<input type="button" value="글쓰기" class="btn" onclick="location.href='galleryUpload.jsp'">
			</div>
			<%
				}
			%>
			<div id="table_search">
				<form action="gallerySearch.jsp" method="get">
			...
			<div id="page_control">
				...
				%><a href="gallery.jsp?pageNum=<%=startPage - pageBlock%>&search=<%=search%>">Prev</a>
				<%
					}
					//  1 2 3 .... 10      11 12 13 .... 20
					for (int i = startPage; i <= endPage; i++) {
				%><a href="gallery.jsp?pageNum=<%=i%>&search=<%=search%>"><%=i%></a>
				<%
					}
					// [다음]
					if (endPage < pageCount) {
				%><a href="gallery.jsp?pageNum=<%=startPage + pageBlock%>&search=<%=search%>">Next</a>
				<%
					}
				%>
			</div>
